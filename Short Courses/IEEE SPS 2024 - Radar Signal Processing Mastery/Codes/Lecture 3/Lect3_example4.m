...
% Listen to the micro-Doppler sound
soundsc(I,sample_rate);
...
for start_index = 1:round(window_size / 4):num_samples - window_size + 1
    ...
    fft_result = 10 * log10(fftshift(magnitude));
    counter = counter + 1;
    waterfall(:, counter) = fft_result;

    % Clear the previous plot and redraw for FFT magnitude
    subplot(2, 1, 1);
    cla; % Clear current axes
    imagesc([1 counter],f, waterfall); % Update imagesc to use frequency axis
